...
def hmi_disambig(azimuth, disambig, method):
    ...
    if azimuth.shape != disambig.shape:
        raise Exception('Dimension of two images do not agree')

    # Ensure disambig is an integer array
    ...
    if len(index[0]) > 0:  # Check if there are any elements to update
        azimuth[index] += 180.  # Update the azimuth values where needed
